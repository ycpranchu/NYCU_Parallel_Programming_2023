...
__global__ void mandelKernel(int* result, int resX, int resY, float lowerX, float lowerY, float dx, float dy, int maxIterations, int pitch)
{
    ...
    int* row = (int*)((byte*)result + thisY * pitch); // char size = 8-bit
    row[thisX] = i;
}
...
void hostFE (float upperX, float upperY, float lowerX, float lowerY, int* img, int resX, int resY, int maxIterations)
{
    ...
    mandelKernel<<<numBlocks, blockSize>>> (result, resX, resY, lowerX, lowerY, dx, dy, maxIterations, pitch);
